  ...
  return (
    <div className="chart-wrap">
      <div className="chart-head">
        <div>
          <div style={{ fontSize: 11.5, color: 'var(--ink-3)', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 4 }}>
            CPO spot · Dumai port
          </div>
          ...
          <div style={{ fontSize: 12, color: 'var(--ink-3)', marginTop: 4 }}>
            4-week average{' '}
            <span className="mono" style={{ color: 'var(--ink-2)' }}>{fmt(Math.round(last4Avg))}</span>
            {' '}· current is{' '}
            <span style={{ color: favorable ? 'var(--green-ink)' : 'var(--amber-ink)', fontWeight: 500 }}>
        ...
        <div className="chart-legend">
          <div className="item">
            <span className="swatch" style={{ background: '#3D8B6E', height: 2 }} />
            CPO price
          </div>
          <div className="item">
            <span className="swatch" style={{ background: 'transparent', borderTop: '1.5px dashed #8A969C', height: 0, width: 14 }} />
            Trend
          </div>
          <div className="item">
            <span className="swatch" style={{ background: 'transparent', borderTop: '1px dashed #5A6B72', height: 0, width: 14 }} />
            4-wk avg
          </div>
        </div>
      </div>
      ...
      <svg className="chart-svg" viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none">
        <defs>
          <linearGradient id="chartFill" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor="#3D8B6E" stopOpacity="0.07" />
        ...
        {ticks.map((v) => (
          <g key={v}>
            <line x1={PAD_L} y1={yFn(v)} x2={W - PAD_R} y2={yFn(v)} stroke="#EAEEEC" strokeWidth="1" />
            <text x={PAD_L - 10} y={yFn(v) + 4} textAnchor="end" fontSize="11" fill="#8A969C" fontFamily="JetBrains Mono, monospace">
              {fmt(v)}
            </text>
          </g>
        ))}
        ...
        <line
          ...
          stroke="#5A6B72" strokeWidth="1" strokeDasharray="2 4"
        />
        <text x={W - PAD_R - 4} y={yFn(last4Avg) - 5} textAnchor="end" fontSize="10.5" fill="#5A6B72">
          4-wk avg
        </text>

        <path d={areaPath} fill="url(#chartFill)" />
        <path d={linePath} fill="none" stroke="#3D8B6E" strokeWidth="2" strokeLinejoin="round" strokeLinecap="round" />

        <line
          x1={xFn(0)} y1={yFn(intercept)}
          x2={xFn(n - 1)} y2={yFn(intercept + slope * (n - 1))}
          stroke="#8A969C" strokeWidth="1.25" strokeDasharray="4 4"
        />
            ...
            {p.current && (
              <>
                <line
                  ...
                  stroke="#3D8B6E" strokeOpacity="0.25" strokeDasharray="2 3"
                />
                <g transform={`translate(${xFn(i)},${yFn(p.price) - 14})`}>
                  <rect x="-32" y="-18" width="64" height="20" rx="4" fill="#0F1A1F" />
                  <text textAnchor="middle" y="-4" fontSize="11" fill="#fff" fontFamily="JetBrains Mono, monospace" fontWeight="500">
                    {fmt(p.price)}
                  </text>
                </g>
              </>
        ...
        <g transform={`translate(${xFn(n - 1)},${H - 26})`}>
          <rect x="-20" y="0" width="40" height="14" rx="3" fill="#3D8B6E" />
          <text textAnchor="middle" y="10" fontSize="10" fill="#fff" fontWeight="600" letterSpacing="0.06em">
            NOW
          </text>
        </g>
      </svg>
